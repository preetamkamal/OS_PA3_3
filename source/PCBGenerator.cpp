...
void PCBGenerator::readnext()
{
    ...
    if (!infile.eof())
    {
        std::stringstream ss;
        std::string line;
        float vals[6]; // Increased size to hold 5 values
        ...
        if (vals[4] < 0 && !error)
            if (error = true)
                std::cout << "I/O Burst time can't be negative. Exiting now." << std::endl;
        ...
        nextPCB = PCB(vals[0], vals[1], vals[2], vals[3], vals[4]); // Updated constructor
    }
    else
        _finished = true;
}
